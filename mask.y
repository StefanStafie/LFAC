...
lista_declarare	: declarare lista_declarare
		...
		| CONSTANT declarare
		;
atribuire	: ID_VAR '<' '-' aritmetic ';'  {$$ = create_int_expr($<strval>1, $4->intvalue);} //numere
...
aritmetic	: aritmetic PLUS numar {$$=create_int_expr(NULL, $1->intvalue + $3->intvalue); free($1); free($3);}
		...
		| numar {$$=create_int_expr("", $1->intvalue);free($1);} //printf("%d\n",$1);}
		;
	...
	for(int i = 0; i<99; i++)
		if(master[i]!= NULL){
			switch(master[i]->type){
				...
				//case 4:
					//printf("Variabila cu numele %s si tipul float are valoarea %f\n", master[i]->nume, master[i]->floatvalue);break;
				default:
