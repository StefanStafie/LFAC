...
atribuire	: ID_VAR '<' '-' aritmetic_int ';'  { $$ = create_int_expr($<strval>1, $4->intvalue); free($4);}//int
		...
		| ID_VAR '[' numar ']' '<' '-' aritmetic_int ';' { update_array($1,$3->intvalue,$7->intvalue);  }
		;
...
void update_array(char* name, int pos, int value )
{
	...
	for(int i=0; i<=99; i++)
		if(master[i]!=NULL && strstr(master[i]->nume,name))
			{  if(master[i]->array[pos]==NULL)
				{master[i]->array[++master[i]->arrayn]=value;}
			   else
